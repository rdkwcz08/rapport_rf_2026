...
This section presents now the full characterization of the one-diode rectifier
...
The circuit consists of the following elements, connected in the series topology
(Figure~2a of the introduction):
...
\begin{itemize}
    \item \textbf{RF source:} a single-tone power source \texttt{P\_1Tone}
          ...
          which converts the input power from dBm to Watts in polar form.

    \item \textbf{DC-blocking / RF choke network:} two capacitors
          $C_1 = C_2 = \SI{100}{\pico\farad}$ placed in series on the signal path
          and in shunt at the output respectively.
          $C_1$ blocks any DC component from reaching the source, while $C_2$
          shorts the RF signal to ground at the output node, acting as an
          RF bypass capacitor that forces the load to see only the rectified DC
          voltage.

    \item \textbf{Schottky diode D1:} the HSMS-2860 model
          (\texttt{di\_hp\_HSMS2860\_20000301}), placed in series between the
    ...
    \item \textbf{Load resistor:} $R_1 = R_L$, representing the equivalent
          input resistance of the target sensor (STTS22H).
          For impedance extraction, $R_L = \SI{10}{\kilo\ohm}$.
\end{itemize}

Two current probes (\texttt{I\_Probe1} on the input path, \texttt{I\_Probe2}
on the output path) monitor the RF input current and the DC output current
respectively.
The LSSP harmonic-balance solver is used rather than a linear $S$-parameter
simulation because the diode is a nonlinear component: its operating point
depends on the RF drive level, making large-signal analysis mandatory.
The impedance $Z_\mathrm{in}$ is extracted via the ADS equation block:
\texttt{Zin1 = zin(S11, PortZ1)}.

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=\linewidth]{figures/schematic_one_diode.png}
    \caption{ADS schematic of the one-diode series rectifier used for
             input impedance extraction.
             The LSSP simulation sweeps from 1 to 3\,GHz.
             D1: HSMS-2860; $C_1 = C_2 = 100$\,pF; $R_L = 10$\,k$\Omega$;
             $P_\mathrm{in} = -20$\,dBm.}
...
The input impedance $Z_\mathrm{in,diode}$ of the HSMS-2860 is extracted using
a Large-Signal $S$-Parameter (LSSP) simulation in Keysight ADS over the
...
$R_L = \SI{10}{\kilo\ohm}$.
The impedance is computed from the $S_{11}$ parameter measured at Port~1
(source impedance $Z_0 = \SI{50}{\ohm}$) using the standard relation:
%
\begin{equation}
    Z_\mathrm{in} = \mathrm{zin}(S_{11}, Z_0)
    = Z_0 \,\frac{1 + S_{11}}{1 - S_{11}}.
    \label{eq:zin_from_s11}
\end{equation}
%
The 100\,pF capacitors act as RF short-circuits at \SI{2.2}{\giga\hertz}
($X_C = 1/(2\pi \times 2.2\times10^9 \times 100\times10^{-12}) \approx
\SI{0.72}{\ohm} \ll \SI{50}{\ohm}$), so they do not affect the RF impedance
measurement while correctly isolating the DC output path.
